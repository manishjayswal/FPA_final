...
def calculation_logic(updated_data, post):
    ...
    if zero_count==33:
        for i in range(1,2):
            ...
            summary_dict['internal_sales_summary_' + yr] = "$"+ str(round(internal_sales_summary, 2))
            
            external_sales_summary = m1.loc['External Sales',starting_month : endind_month].sum() / 1000000
            summary_dict['external_sales_summary_' + yr] = "$"+ str(round(external_sales_summary, 2))

            sales_summary = m1.loc['Total Sales', starting_month : endind_month].sum() / 1000000
            summary_dict['sales_summary_' + yr] = "$"+ str(round(sales_summary, 2))

            payment_rate_summary = m1.loc['Payments',starting_month : endind_month].sum() / m1.loc['BOP Balance', starting_month : endind_month].sum()
            summary_dict['payment_rate_summary_' + yr] = str(round(payment_rate_summary*100, 2))+"%"
            
            enr_summary = m1.loc['EOP Balance', endind_month] / 1000000
            summary_dict['enr_summary_' + yr] = "$"+ str(round(enr_summary, 2))

            
            anr_summary = m1.loc['Avg Balance', starting_month : endind_month].mean() / 1000000
            summary_dict['anr_summary_' + yr] = "$"+ str(round(anr_summary, 2))

            merchant_sales_summary = m1.loc['Merchant Sales', starting_month : endind_month].sum() / 1000000
            summary_dict['merchant_sales_summary_' + yr] = "$"+ str(round(merchant_sales_summary, 2))

            credit_penetration_summary = internal_sales_summary / merchant_sales_summary
            summary_dict['credit_penetration_summary_' + yr] = str(round(credit_penetration_summary*100, 2)) + "%"
            
            transactions_k_summary = m1.loc['Transactions',starting_month : endind_month].sum()/1000
            summary_dict['transactions_k_summary_' + yr] = round(transactions_k_summary, 2)
            
            average_ticket_summary = 1000 * sales_summary / transactions_k_summary    
            summary_dict['average_ticket_summary_' +yr] = "$"+ str(round(average_ticket_summary, 2))

            new_accounts_summary = m1.loc['New Accounts', starting_month : endind_month].sum()
            summary_dict['new_accounts_summary_' +yr] = round(new_accounts_summary, 2)
            
            eop_accounts_summary = m1.loc['EOP Accounts', endind_month]
            summary_dict['eop_accounts_summary_' +yr] = round(eop_accounts_summary, 2)
            
            applications_summary = m1.loc['Applications',starting_month : endind_month].sum()
            summary_dict['applications_summary_' +yr] = round(applications_summary, 2)
            
            approval_rate_summary = new_accounts_summary / applications_summary
            summary_dict['approval_rate_summary_' +yr] = str(round(approval_rate_summary*100, 2)) + "%"
            
            average_actives_summary = m1.loc['Active Accounts', starting_month : endind_month].mean()
            summary_dict['average_actives_summary_' +yr] = round(average_actives_summary, 2)
            
            sales_per_active_summary = 1000000 * sales_summary / average_actives_summary
            summary_dict['sales_per_active_summary_' +yr] = "$"+ str(round(sales_per_active_summary, 2))
            
            balance_per_active_summary = 1000000 * anr_summary / average_actives_summary
            summary_dict['balance_per_active_summary_' +yr] = "$"+ str(round(balance_per_active_summary, 2))
            
            active_rate_summary = average_actives_summary / eop_accounts_summary
            summary_dict['active_rate_summary_' +yr] = str(round(active_rate_summary*100, 2)) + "%"
            ...
            summary_dict['floating_apr_summary_' +yr] = str(round(floating_apr_summary*100, 2)) + "%"
            ...
            summary_dict['fixed_apr_summary_' +yr] = str(round(fixed_apr_summary*100, 2)) + "%"
            ...
            summary_dict['promo_apr_summary_' +yr] = str(round(promo_apr_summary*100, 2)) + "%"
            ...
            summary_dict['transactors_summary_' +yr] = str(round(transactors_summary*100, 2)) + "%"
